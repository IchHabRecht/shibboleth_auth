...
class tx_shibbolethauth_pi1 extends AbstractPlugin
{
    ...
    protected function showLoginError()
    {
        ...
        if (!empty($_SERVER[$this->extConf['remoteUser']])) {
            $markerArray['###ERROR_MESSAGE###'] .= '<br>' . str_replace('###USER###', $this->remoteUser, $this->pi_getLL('wrong_user', '', 1));
        }
            ...
            foreach ($getVars as $key => $val) {
                if (stristr($key, $this->prefixId) === false) {
                    if (is_array($val)) {
                        ...
                        if (!in_array($key, ['id', 'no_cache', 'logintype', 'redirect_url', 'cHash'])) {
                            $params .= '&' . $key . '=' . $val;
                        }
                    }
                }
    ...
    private function initPIflexForm()
    {
        ...
        $this->lConf = []; // Setup our storage array...
